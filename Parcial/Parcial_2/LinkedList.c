...
LinkedList* H_Filter(LinkedList* this, int (*pFuncion) (void*) )
{
    LinkedList* listaFiltrada = NULL; // DECLARA LISTA NUEVA

    int i;
    int len = this->size; // TOMA EL TAMANIO DE THIS
    void* aux;
    if(this != NULL && pFuncion != NULL && len > 0) // COMPRUEBA QUE THIS, EL PUNTERO A LA FUNCION NO SEAN NULOS Y LEN MAYOR A 0
    {
        ...
        if(listaFiltrada != NULL)
        {
            for(i=0; i<len; i++) // RECORRE LA LISTA
            {
                ...
                if(pFuncion(aux) == 1) // ENVIA A LA FUNCION EL NODO SI AL VOLVER DE ESA FUNCION EL RESULTADO ES POSITIVO SE GUARDA EN LA LISTA FILTRADA
                {
