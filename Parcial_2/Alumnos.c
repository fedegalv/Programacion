...
int saveAsText(char* path, LinkedList* arrayEmpleados,int isValid, int promFinal)
{
    if(isValid == 1)
    {
        ...
        auxAlumno= alumno_new();
        printf("\nCopiando a archivo...\n");
        printf("PROMEDIO DE TODOS LOS ALUMNOS: %d\n", promFinal);
        printf("Se guardaran el un archivos los alumnos con notas finales superiores al promedio de todos los alumnos.\n\n");
        ...
        for(i=0; i < listLen; i++)
        {
            ...
            if(auxAlumno->notaFinal > promFinal)
            {
                ...
                printf("Alumno %d supero el promedio, escribiendo en archivo..\n", auxAlumno->id);
...
int calcularPromedio(LinkedList* arrayAlumnos)
{
    ...
    for(i=0; i < listLen; i++)
    {
        ...
        acumNotasTotal= acumNotasTotal+ auxAlumno->notaFinal;

    }
    promedio= acumNotasTotal / listLen;
    printf("Calculando promedio...\n");
    printf("El promedio de todos los alumnos es: %d\n", promedio);
    return promedio;
}

int asignacionNota(LinkedList* arrayAlumno, void* aux, int cont) // DEVUELVE 0 SI SALIO TODO BIEN
{
    int setOK;
    int randomNumber;
    sAlumno* auxAlumno;
    ...
    setOK= ll_set(arrayAlumno, cont, auxAlumno);
...
int alumnosSuperanPromedio(LinkedList* arrayAlumnos,int index, int promedioFinal)
{
    int retorno= 0;
    if( arrayAlumnos != NULL)
    {
        sAlumno* auxAlumno;
            ...
            auxAlumno= ll_get(arrayAlumnos, index);
            if(auxAlumno->notaFinal > promedioFinal )
            {
                retorno= 1;
            }
    }
